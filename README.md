...
```
 ...
 6. 视频 → GIF
 7. 视频 → MP3
 操作流程：选择功能 → 拖入文件 → GPU加速(可选) → 选择输出目录 → 设置裁剪(可选) → 确认预览 → 自动转换
```
...
| GPU 加速 | 启用后自动使用 h264_nvenc / h264_amf / h264_qsv |
| 视频质量 | CRF 0-51（0=无损，23=默认，51=最低） |
| 图片质量 | 2-31（2=最高，31=最低） |
| 帧率 | 30、60 等 |
| 码率 | `2M`、`5000k` 等 |
| 编码预设 | ultrafast ~ veryslow（GPU 自动映射为 p1~p7 等） |
| 视频裁剪 | 起始时间 + 持续时长（回车跳过） |
...
- 视频编码：H.264 (libx264)、NVENC、AMF、QSV、H.265、VP9、Xvid 等
